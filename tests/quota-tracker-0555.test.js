...
assert.ok(!usage.includes("qtpDeepSeek"), "0.5.55 must keep official DeepSeek usage");
assert.ok(!usage.includes("qtpNormalizeXai"), "0.5.55 must keep official Grok usage unwrapped");
...
assert.ok(
  ...
  "0.5.55 catalog must use native alitp-intl instead of injecting qwen-cloud-token-plan",
);
...
assert.ok(
  ...
  "0.5.55 client catalog must not inject qwen-cloud-token-plan",
);
...
assert.ok(
  ...
  "0.5.55 615.js must not inject qwen-cloud-token-plan",
);

...
const originals = path.join(
  repoRoot,
  "patches/quota-tracker-originals/enhanced-0.5.55",
);
const patcher = path.join(repoRoot, "patches/quota-tracker.patch.js");

assert.ok(fs.existsSync(originals), "enhanced-0.5.55 originals must exist");
assert.ok(fs.existsSync(patcher), "quota-tracker.patch.js must exist");

const scratch = fs.mkdtempSync(path.join(os.tmpdir(), "quota-0555-enh-"));
...
assert.equal(checkBefore.catalogVariant, "enhanced-0.5.55");
...
assert.equal(checkAfter.catalogVariant, "enhanced-0.5.55");
...
console.log("quota tracker enhanced-0.5.55 isolation tests: ok");

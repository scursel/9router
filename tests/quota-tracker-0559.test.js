...
const originals = path.join(
  repoRoot,
  "patches/quota-tracker-originals/official-0.5.59",
);
const patcher = path.join(repoRoot, "patches/quota-tracker.patch.js");

assert.ok(fs.existsSync(originals), "official-0.5.59 originals must exist");
assert.ok(fs.existsSync(patcher), "quota-tracker.patch.js must exist");

const scratch = fs.mkdtempSync(path.join(os.tmpdir(), "quota-0559-"));
...
assert.equal(checkBefore.catalogVariant, "official-0.5.59");
...
assert.equal(checkAfter.catalogVariant, "official-0.5.59");
...
assert.ok(usage.includes("qtpOpenCodeGo"), "0.5.59 usage chunk must include OpenCode Go collector");
...
assert.ok(!usage.includes("qtpDeepSeek"), "0.5.59 must keep official DeepSeek usage");
assert.ok(!usage.includes("qtpNormalizeXai"), "0.5.59 must keep official Grok usage unwrapped");
...
assert.ok(
  ...
  "0.5.59 must keep the official Antigravity collector",
);
assert.ok(
...
assert.ok(
  ...
  "0.5.59 must keep the official OpenCode Go model catalog",
);
assert.ok(
  !catalog.includes('"id":"qwen-cloud-token-plan"') && !catalog.includes('id:"qwen-cloud-token-plan"'),
  "0.5.59 catalog must use native alitp-intl instead of injecting qwen-cloud-token-plan",
);
...
assert.ok(
  ...
  "0.5.59 client catalog must not inject qwen-cloud-token-plan",
);
...
assert.ok(
  ...
  "0.5.59 615.js must not inject qwen-cloud-token-plan",
);

const ui = fs.readFileSync(
  path.join(
    scratch,
    "app/.next-cli-build/static/chunks/app/(dashboard)/dashboard/quota/page-69f67dfea276e5ad.js",
  ),
  "utf8",
);
assert.ok(ui.includes("/* QuotaTrackerCurrency:v2 */"), "0.5.59 dashboard must render USD currency");
...
assert.equal(checkClean.catalogVariant, "official-0.5.59");
assert.equal(checkClean.usagePatched, false);
assert.equal(checkClean.catalogPatched, 0);

fs.rmSync(scratch, { recursive: true, force: true });
console.log("quota tracker official-0.5.59 isolation tests: ok");
